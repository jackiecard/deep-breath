...
function OnGUI () {
	...
	GUI.color.a = -offset;
	if(GUI.color.a > 0.0 && Mathf.Round(swipeCtrl.smoothValue) - 1 >= 0 && Mathf.Round(swipeCtrl.smoothValue) - 1 < img.length) {
		GUI.DrawTexture(new Rect(mainPos - imgRect.width, imgRect.y, imgRect.width, imgRect.height), img[Mathf.Round(swipeCtrl.smoothValue) - 1]);
	}
	GUI.color.a = offset;
	if(GUI.color.a > 0.0 && Mathf.Round(swipeCtrl.smoothValue) + 1 < img.length && Mathf.Round(swipeCtrl.smoothValue) + 1 >= 0) {
		GUI.DrawTexture(new Rect(mainPos + imgRect.width, imgRect.y, imgRect.width, imgRect.height), img[Mathf.Round(swipeCtrl.smoothValue) + 1]);
	}
	GUI.color.a = 1.0;
