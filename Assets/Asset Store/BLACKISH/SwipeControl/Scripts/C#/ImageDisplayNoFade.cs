...
public class ImageDisplayNoFade : MonoBehaviour {
	...
	public int numberOfImagesVisibleLeft = 1; //how many additional images should be shown left of the selected one?
	public int numberOfImagesVisibleRight = 3; //how many additional images should be shown right of the selected one?
	public int distanceBetweenImages = 0; //how many pixels in between images?
	...
	void Awake () {
		...
		if(expandInputAreaToFullWidth) {
			swipeCtrl.SetMouseRect(new Rect(-Screen.width, imgRect.y, Screen.width * 2, imgRect.height)); // Use image-height for the input-Rect, but full screen-width
		} else {
	...
	void OnGUI () {
		...
		float mainPos = imgRect.x - (offset * (imgRect.width + distanceBetweenImages));
		
		for(int n = -numberOfImagesVisibleLeft; n <= numberOfImagesVisibleRight; n++) {		
			if(Mathf.Round(swipeCtrl.smoothValue) + n >= 0 && Mathf.Round(swipeCtrl.smoothValue) + n < img.Length) {
				GUI.DrawTexture(new Rect(mainPos + imgRect.width * n + distanceBetweenImages * n, imgRect.y, imgRect.width, imgRect.height), img[(int) Mathf.Round(swipeCtrl.smoothValue) + n]);
			}
		}
