...
public class ImageDisplayXYHorizontal : MonoBehaviour {
	...
	//OPTIONS
	public bool displayAll = false;	//Display all images, don't fade out the unselected ones...
	...
	void OnGUI () {
		...
		if(displayAll) { //display all images
			...
			for(int i = 0; i < img.Length; i++) {
				GUI.DrawTexture(new Rect(imgRect.x + imgRect.width * i - swipeCtrl.smoothValue.x * imgRect.width, imgRect.y, imgRect.width, imgRect.height), img[i]);
			}
			...
			if(Mathf.Round(swipeCtrl.smoothValue.x) >= 0 && Mathf.Round(swipeCtrl.smoothValue.x) < img.Length) {
				...
				GUI.DrawTexture(new Rect(mainPos, imgRect.y, imgRect.width, imgRect.height), img[(int) Mathf.Round(swipeCtrl.smoothValue.x)]);
			}
			...
			if(GUI.color.a > 0.0f && Mathf.Round(swipeCtrl.smoothValue.x) - 1 >= 0 && Mathf.Round(swipeCtrl.smoothValue.x) - 1 < img.Length) {
				GUI.DrawTexture(new Rect(mainPos - imgRect.width, imgRect.y, imgRect.width, imgRect.height), img[(int) Mathf.Round(swipeCtrl.smoothValue.x) - 1]);
			}
			GUI.color = new Color(1f, 1f, 1f, offset);
